...
switch($method) {
    case 'POST':
        ...
        if (!empty($data->cardId) && !empty($data->tag)) {
            ...
            $query = "INSERT IGNORE INTO card_tags (card_id, tag) VALUES (?, ?)";
            $stmt = $db->prepare($query);
            $stmt->bindParam(1, $card_id);
            $stmt->bindParam(2, $data->tag);
    ...
    case 'DELETE':
        ...
        if (!empty($data->cardId) && !empty($data->tag)) {
            ...
            $query = "DELETE FROM card_tags WHERE card_id = ? AND tag = ?";
            $stmt = $db->prepare($query);
            $stmt->bindParam(1, $card_id);
            $stmt->bindParam(2, $data->tag);
